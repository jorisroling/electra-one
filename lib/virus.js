...
class Virus {
  ...
  static scanBanks() {
    ...
    for (let bankFile of bankFiles) {
      ...
      for (let p = presetCount - 1; p >= 0; p--) {
        ...
        if (preset) {
          if (nonEmptyFound || (!preset.name.match(/^\s*\W\s*init\s*\W\s*$/i) && !preset.name.match(/empty/i))) {
            if (name == "001 Init Multis" || name == "001 Init Singles" || name == "jj") {
     ...
     if (name == "001 Init Multis" || name == "001 Init Singles") {
        console.log(name,presetCount,ditched)
     }
    }
