...
class Virus {
      ...
      if (virusPreset) {
        ...
        if (bytes) {
          if (stateDiff) {
            ...
            debug('NOT Send Virus Preset over MIDI %y for part %y', virusPreset.name, part)
          }
  ...
  static scanBanks() {
        ...
        if (preset) {
          if (nonEmptyFound || (!preset.name.match(/^\s*\W\s*init\s*\W\s*$/i) && !preset.name.match(/empty/i))) {
            ...
            categoryOffsets.forEach(categoryOffset => {
              const catName = Virus.categroryName(Virus.getPresetPageParameter(preset, 1, categoryOffset))
              if (catName) {
                _.set(libraryCategories, catName, _.get(libraryCategories, catName, 0) + 1)
              }
            })
            Virus.categroryName(Virus.getPresetPageParameter(preset, 1, categoryOffsets[1]))
          } else {
    ...
    /*    _.set(bacaraPresetState, 'virus.library.banks', banks)*/
    const cnt = banks.reduce((partial_sum, bank) => partial_sum + bank.presetCount, 0)
    //    _.set(bacaraPresetState, 'virus.library.presetCount', cnt)
    Bacara.setPresetState('virus.library.presetCount', cnt)
    debug('Presets %y', cnt)
    //    _.set(bacaraPresetState, 'virus.library.categories', libraryCategories)
  ...
  static getBanks(callback) {
    const bacaraPresetState = Bacara.presetState()
    if (callback) {
      callback(_.get(bacaraPresetState, 'virus.library.banks'))
    } else {
      return _.get(bacaraPresetState, 'virus.library.banks')
    }
  }
  ...
  static presetToSysEx(part, preset, bank, program) {
    ...
    debug ('Preset part %y bank %y program %y name %y (%y, %y) filename %y', part, bank, program, preset.name, Virus.categroryName(Virus.getPresetPageParameter(preset, 1, categoryOffsets[0])), Virus.categroryName(Virus.getPresetPageParameter(preset, 1, categoryOffsets[1])), preset.filename)
  ...
  static searchCategory(category, direction, bank, program, callback) {
    if (!direction) {
      direction = Machine.getRandomInt(1) ? 1 : -1
    }
    //    debug('Search category %y (%y) direction %y  bank %y  program %y', category, Virus.categroryName(category), direction > 0 ? 'up' : 'down', bank, program)
    const bacaraPresetState = Bacara.presetState()
    debug('Search category %y (of total %y) direction %y  from bank %y  program %y onward', Virus.categroryName(category), _.get(bacaraPresetState, `virus.library.categories.${Virus.categroryName(category)}`), direction > 0 ? 'up' : 'down', bank, program)
    Virus.getBanks( banks => {
      if (banks) {
        bank = (bank >= 0) ? bank : 0
  ...
  static randomBankAndProgram(callback) {
    Virus.getBanks( banks => {
      if (banks && banks.length) {
        ...
        const program = Machine.getRandomInt(banks[bank].presetCount - 1)
        callback && callback(bank, program)
      }
    })
  }

  static categroryName(cat) {
