...
const Virus = {
  ...
  scanBanks() {
    ...
    for (let bankFile of bankFiles) {
      ...
      banks.push({index: bankIdx++,filename:bankFile.substr(bankPath.length+1), name, short, size:fileSizeInBytes, presets, residu: fileSizeInBytes - (presets*presetInBankSize)})
    }
