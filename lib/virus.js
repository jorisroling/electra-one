...
class Virus {
  ...
  static scanBanks() {
    const banks = []
    if (!_.get(config,`virus.bankPath`)) return
    ...
    _.set(virusState, 'library.banks', banks)
    const cnt = banks.reduce((partial_sum, bank) => partial_sum + bank.presetCount, 0)
    _.set(virusState, 'library.presetCount', cnt)
    debug('Presets %y', cnt)
    _.set(virusState, 'library.categories', libraryCategories)

    fs.ensureDirSync(path.dirname(filePath))
    jsonfile.writeFileSync(filePath, virusState, { flag: 'w', spaces: 2 })
    debug('Scan stored in %y', filePath)
  ...
  static getBanks(callback) {
    if (callback) {
      callback(_.get(virusState, 'library.banks'))
    } else {
      return _.get(virusState, 'library.banks')
    }
    ...
    if (fs.existsSync(filePath)) {
      if (callback) {
        jsonfile.readFile(filePath, (err, virusState) => {
...
*/
  ...
  static searchCategory(category, direction, bank, program, callback) {
    ...
    debug('Search category %y (of total %y) direction %y  from bank %y  program %y onward', Virus.categroryName(category), _.get(virusState, `library.categories.${Virus.categroryName(category)}`), direction > 0 ? 'up' : 'down', bank, program)
    Virus.getBanks( banks => {
      if (banks) {
        bank = (bank >= 0) ? bank : 0
