...
//      console.log('hi',presetStateFilePath)
//      console.trace('jjr')
    }
    return presetState
  }
...
//      console.log('write')
  ...
  // Read the presetSate through config (which merged the presetState file into it)
  static getPresetState(path, dflt) {
    const bacaraPresetState = Bacara.presetState()
    return _.get(config, `preset.${path}`, dflt)
  }

  // Write to the presetSate & write file, should only be called by utility commands
