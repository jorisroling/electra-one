...
/*    return midiFile*/
  ...
  load_pattern(state) {
    ...
    //    debug('load_pattern dir %y', dir)
    const patternFiles = glob.sync(dir, {})
    ...
    if (patternFiles[(patternFiles.length - 1) - but]) {
      ...
      const ticksPerStep = 120
      let steps = 1
      while (Math.floor(maxTicks/ticksPerStep)>steps) steps *= 2
      _.set(state,'patternSteps',steps)
