...
function generateMidi(pattern, steps) {
  if (Array.isArray(pattern)) {
    ...
    for (let i = 0; i < instruments.length; i++) {
      //      if (1 || ( i < pattern.length && Array.isArray(pattern[i]))) {
      const instrument = instruments[i]
      const pitch = i
      const velLow = VELOCITY_LO
      const velHigh = VELOCITY_HI
      for (let s = 0; s < steps; s++) {
        //          if (1 || ( s < pattern[i].length && Array.isArray(pattern[i][s]))) {
        const idx = 11 + s * 5
...
class Drums {
  ...
  static midiFromPatterns(steps, patterns) {
    ...
    if (writeMidiFiles) {
      ...
      return JSON.parse(JSON.stringify(midiJson))
    } else {
      return midiFile
    }
  }
