...
const Electra = {
  ...
  parseSysexCmdPatchRequestResponse(portName, bytes) {
    debug('parseSysexCmdPatchRequestResponse  port %y  bytes %y', portName, bytes && bytes.length)
    ...
    if (match && match.length && match[1]) {
      ...
      return _.get(cache, `${portName}.preset`)
    }
  },
  ...
  parseSysexCmdInfoResponse(portName, bytes) {
    debug('parseSysexCmdInfoResponse  port %y  bytes %y', portName, bytes && bytes.length)
    let json
    try {
      const data = bytes.slice(6, bytes.length - 1).reduce((a, c) => a + String.fromCharCode(parseInt(c)), '')
      json = JSON.parse(data)
