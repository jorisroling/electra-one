...
class Interface  {
  ...
  getParameter(path, kind = 'internal') {  // internal, surface, external, modulated
    ...
    if (kind == 'surface' || kind == 'external') {
      ...
      debug('getParameter path %y result %y x1 %y y1 %y x2 %y y2 %y = %y', path, result, x1, y1, x2, y2, Math.round(remap(result, x1, y1, x2, y2)))
  ...
  setParameter(path, value, origin = 'internal') {
    ...
    if (origin == 'surface' || origin == 'external') {
      ...
      debug('setParameter path %y value %y x1 %y y1 %y x2 %y y2 %y = %y', path, value, x1, y1, x2, y2, remap(value, x1, y1, x2, y2))
  ...
  connect(portName, kind, channel = 0, incomming = true, outgoing = true) {
    ...
    const connection = {portName, kind, channel, incomming, outgoing}
