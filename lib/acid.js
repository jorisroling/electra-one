...
const Acid = {
  generate(state) {
    ...
    const filepath = path.resolve((process.env.NODE_ENV == 'production') ? untildify(`~/.electra-one/state/acid/patterns/${name.replace(/:/g,'.')}.mid`) : `${__dirname}/../state/acid/patterns/${name.replace(/:/g,'.')}.mid`)
  ...
  load_pattern(state) {
    ...
    const patternFiles = glob.sync(path.resolve(
      ( (process.env.NODE_ENV == 'production') ? path.join(untildify('~/.electra-one'),'state','acid','patterns') : path.join(__dirname,'..','state','acid','patterns') ) + '/*.mid'), {})
  ...
  load_preset(state) {
    ...
    const presetFiles = glob.sync(path.resolve(
      ( (process.env.NODE_ENV == 'production') ? path.join(untildify('~/.electra-one'),'state','acid','presets') : path.join(__dirname,'..','state','acid','presets') ) + '/*.json'), {})
    ...
    if (presetFiles[(presetFiles.length - 1) - but]) {
      const values = jsonfile.readFileSync(presetFiles[(presetFiles.length - 1) - but])
      if (values) {
  ...
  save_preset(state) {
    const name = `Acid - ${new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '')}`
    const filePath = path.resolve((process.env.NODE_ENV == 'production') ? untildify(`~/.electra-one/state/acid/presets/${name.replace(/:/g,'.')}.json`) : `${__dirname}/../state/acid/presets/${name.replace(/:/g,'.')}.json`)
    fs.ensureDirSync(path.dirname(filePath))
