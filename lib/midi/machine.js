...
class Machine extends EventEmitter  {
  ...
  setState(path, value, section, emitChanges = true) {
    ...
    if (section > 0) {
      if (typeof _.has(this.interface,`sections.${String.fromCharCode(64+section)}.state`) != 'object') _.set(this.interface,`sections.${String.fromCharCode(64+section)}.state`,{})
      target = this.interface.sections[String.fromCharCode(64+section)].state
//      if (section==6) console.log(target)
    }
//    if (section==6) console.log('hi',section,path)
    _.set(target, path, value)
//      if (section==6) console.log(this.getState(path,null,section))
    if (emitChanges) {
