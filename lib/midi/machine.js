...
class Machine extends EventEmitter  {
  ...
  constructor(name, windowEmitter) {
    ...
    this.name = name
    this.interface = new Interface(name, windowEmitter)
    this.connections = {}
    this.state = {}
  ...
  connect(portName, kind, channel = 0, incoming = true, outgoing = true, active = true) {
    ...
    const result = this.interface.connect(portName, kind, channel, incoming, outgoing, active)
    this.connections[kind] = result
    return result
  }

  setConnectionActive(kind, active) {
    const connection = this.connections[kind]
    if (connection) {
      connection.setActive(active)
    }
    debug('Connection %y is %sactive', kind, active ? '' : 'NOT ')
  }
  ...
  setState(path, value, variant, emitChanges = true) {
    ...
    if (variant > 0) {
      if (typeof _.get(this.interface, `variants.${String.fromCharCode(64 + variant)}.state`) != 'object') {
        _.set(this.interface, `variants.${String.fromCharCode(64 + variant)}.state`, {})
      }
