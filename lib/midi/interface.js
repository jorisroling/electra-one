...
*/
  ...
  setParameter(elementPath, value, origin = 'internal', section, emitChanges = true) {
    ...
    if (type == 'parameter' || type == 'feedback') {
      ...
      if ( this.getParameter(elementPath) !== value ) {

        if (origin != 'surface') {
          const timestamp = _.get(this.journal, `${elementPath}.surface`)
          if (timestamp) {
            if ((Date.now() - timestamp) < SmoothSurfaceControlTimeWindow) {
              debug('Smooth Ignore %y = %y (current %y) (origin %y) delta %y ms', elementPath, value, this.getParameter(elementPath), origin, Date.now() - timestamp)
              return
            }
          }
        }
        ...
        if (section>0 && this.getParameter(elementPath,null,0) === value) {
...
//                debug('B:after %y',this.getParameter(path, 'internal', section))
              }
            }
          } else {
            const templateKeys = Object.keys(template)
