...
//        console.log('---',variant,jj,value,originalValue)
        ...
        if (variant == this.getParameter('variant','internal',0) || variant == elVariant || !_.has(this,`variants.${String.fromCharCode(64+variant)}.parameters.${elementPath}`)) {
          ...
          if (this.windowEmitter) {
            debug('send (to %y) %y (%y) = midi %y (intern %y)', 'window', elementPath, this.getElementAttribute(elementPath, 'name', elementPath), this.getParameter(elementPath, 'window'), this.getParameter(elementPath))
...
//               debug('B:after (%y) %y = %y',variant, path, this.getParameter(path, 'internal', variant))
  ...
  sendValues(target) {
    ...
    if (this.windowEmitter) {
      this.iterateElelements((template, path) => {
        debug('send (to %y) %y (%y) = midi %y (intern %y)', 'window', path, this.getElementAttribute(path, 'name', path), this.getParameter(path, 'window'), this.getParameter(path))
