...
*/
  ...
  getParameter(elementPath, kind = 'internal', section) {  // internal, surface, external, modulated
    ...
    if (elementPath == 'section') {
      section = 0
    } else {
      if (typeof(section) == 'undefined') {
        section = this.getParameter('section')
      }
      if (section > 0) {
        if (this.sections && _.has(this.sections,`${String.fromCharCode(64+section)}.parameters.${elementPath}`)) {
          source = this.sections[String.fromCharCode(64+section)].parameters
        }
      }
    }
    const type = this.getElementAttribute(elementPath, 'type')
  ...
  setParameter(elementPath, value, origin = 'internal', section, emitChanges = true) {
    let target = this.parameters
    if (elementPath == 'section') {
      section = 0
    } else {
      if (typeof(section) == 'undefined') {
        section = this.getParameter('section','internal',0)
      }
      if (section > 0) {
        if (!_.has(this,`sections.${String.fromCharCode(64+section)}.parameters`)) _.set(this,`sections.${String.fromCharCode(64+section)}.parameters`,{})
        target = this.sections[String.fromCharCode(64+section)].parameters
      }
    }
    ...
    if (type == 'parameter' || type == 'feedback') {
      ...
      if ( this.getParameter(elementPath,origin,section) !== value ) {

        if (origin != 'surface') {
          const timestamp = _.get(this.journal, `${elementPath}.surface`)
          if (timestamp) {
            if ((Date.now() - timestamp) < SmoothSurfaceControlTimeWindow) {
              debug('Smooth Ignore %y = %y (current %y) (origin %y) delta %y ms', elementPath, value, this.getParameter(elementPath,origin,section), origin, Date.now() - timestamp)
              return
            }
          }
        }
        if (!this.getElementAttribute(elementPath, 'silent', false) && this.connections && this.connections.length) {
          ...
          debug('received (from %y) (section %y) %y (%y) = %s %s', origin, section, elementPath, name ? name : elementPath, val, this.getElementAttribute(elementPath, 'unit', ''))
        }
        ...
        const originalValue = this.getParameter(elementPath,origin,section)

        if (section>0 && this.getParameter(elementPath,origin,0) === value) {
...
*/
  ...
  getSectionParameters(section) {
    return _.get(this,`sections.${String.fromCharCode(64+section)}.parameters`,{})
  }

  setSectionParameters(section, source) {
    if (_.isPlainObject(source)) {
      const copySelective = (source, template, path = '') => {
        if (template) {
          if (Interface.isEdge(template)) {
...
//               debug('B:(%y) %y = %y',section,path,source)
               this.setParameter(path, source, 'internal', section)
//               debug('B:after (%y) %y = %y',section, path, this.getParameter(path, 'internal', section))
              }
            }
          } else {
            const templateKeys = Object.keys(template)
      ...
      _.set(this,`sections.${String.fromCharCode(64+section)}.parameters`,{})
      copySelective(source, this.interface.elements)
    } else {
      debug('not plain object',typeof source,source)
    }
    debug('final section %y %y',String.fromCharCode(64+section),_.get(this,`sections.${String.fromCharCode(64+section)}.parameters`,{}))
  }

  getSectionState(section) {
    return _.get(this,`sections.${String.fromCharCode(64+section)}.state`,{})
  }

  setSectionState(section, source) {
    _.set(this,`sections.${String.fromCharCode(64+section)}.state`,source)
  }
