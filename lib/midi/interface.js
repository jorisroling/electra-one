    ...
    if (type == 'parameter' || type == 'feedback') {
      ...
      if ( _.get(this.parameters, path) !== value ) {
        ...
        if (!this.getElementAttribute(path, 'silent', false) && this.connections && this.connections.length) {
          ...
          debug('received (from %y) %y (%y) = %s %s', origin, path, name ? name : path, val, this.getElementAttribute(path, 'unit', ''))
        }
        ...
        const originalValue = _.get(this.parameters, path)
        ...
        if (origin != 'internal' && emitChanges) {
          this.emit('parameterChange', path, value, origin, originalValue)
        }
        return true // CHANGED
      }
    } else {
      debug('setParameter: Element %y is not a parameter (it is a %y)', path, type)
    }
