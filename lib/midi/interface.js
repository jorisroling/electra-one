    ...
    if (this.windowEmitter) {
      this.windowEmitter.on('message', (args) => {
        ...
        if (typeof args == 'object') {
          switch (args.type) {
          case 'getInterface':
            this.windowEmitter.send('interface', {interface:this.interface})
            break
          case 'getInterfaceAndParameters':
            this.windowEmitter.send('interfaceAndParameters', {interface:this.interface, parameters:this.parameters})
            break
          case 'setParameter':
            this.setParameter(args.path, args.value, 'window')
            break
          case 'triggerAction':
            /*              console.log('triggerAction',args.path)*/
            this.triggerAction(args.path, 'window')
            break
          }
        }
      })
...
*/
  ...
  static isEdge(node) {
    return _.isPlainObject(node) && (node.surface || node.external || node.window) && node.type && node.name
  }
  ...
  getParameter(elementPath, kind = 'internal', variant) {  // internal, surface, external, modulated, window
    ...
    if (type == 'parameter' || type == 'feedback') {
      ...
      if (kind == 'surface' || kind == 'external' || kind == 'window') {
  ...
  setParameter(elementPath, value, origin = 'internal', variant, emitChanges = true) {
    ...
    if (elVariant >= 0) {
      ...
      if (variant > 0) {
        if (!_.has(this, `variants.${String.fromCharCode(64 + variant)}.parameters`)) {
    ...
    if (type == 'parameter' || type == 'feedback') {
      ...
      if (origin == 'surface' || origin == 'external' || origin == 'window') {
