...
const sendShadowNotesToBacaraPort = true
...
//        console.trace('JJR')
  ...
  static send(portName, type, options, label = null, timeout = 0) {
    ...
    if (portName == 'virus-ti' && !config.virus.enabled) {
//      console.trace('Virus TI  type '+type)
      return
    }
    portName = this.normalisePortName(portName)
    //    debug('. %y',options.valueMsb)
    if (portName == this.normalisePortName(bacaraVirtualName) && ['noteon', 'noteoff'].indexOf(type) < 0) {
      return
    }
    ...
    function sendDeduped(portName, type, options) {
      ...
      if (midiOutput) {
        ...
        debug('Unable to send %y to port %y',type,portName)
      }
    }

    if (type != 'sysex' && !label && !timeout) {
      sendDeduped(portName, type, options)
      if (sendShadowNotesToBacaraPort && portName != this.normalisePortName(bacaraVirtualName) && (type == 'noteon' || type == 'noteoff')) {
