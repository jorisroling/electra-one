...
class MidiConnection {
  constructor(portName, kind, channel, incoming, outgoing, interfce) {
  ...
  sendParameter(path, label = null, timeout = 0) {
    ...
    if (type) {
      if (type == 'nrpn') {
        ...
        Midi.send(this.portName, type, {channel:this.channel, number, value:val}, label, timeout)
      }
      if (type == 'cc') {
        ...
        if (hiRes && number >= 0 && number < 32) {
          const lsbFirst = this.interface.getElementAttribute(path, `${this.kind}.lsbFirst`, false)
          if (lsbFirst) {
            Midi.send(this.portName, type, {channel: this.channel, controller:number + 32, value:(value >> 7) & 0x7F}, label, timeout)
            Midi.send(this.portName, type, {channel: this.channel, controller:number, value:(value >> 0) & 0x7F}, label, timeout)
          } else {
            Midi.send(this.portName, type, {channel: this.channel, controller:number, value:(value >> 7) & 0x7F}, label, timeout)
            Midi.send(this.portName, type, {channel: this.channel, controller:number + 32, value:(value >> 0) & 0x7F}, label, timeout)
          }
        } else {
  ...
  handleMidi() {
    ...
    return (msg) => {
      ...
      if (msg._type == 'cc') {
        this.midiCache.setValue(this.portName, msg.channel, 'cc', msg.controller, msg.value)
