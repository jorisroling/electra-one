  ...
  sendParameter(path) {
    ...
    if (type) {
      if (type == 'nrpn') {
        ...
        Midi.send(this.portName, type, {channel:this.channel, number, value:val})
      }
      if (type == 'cc') {
        ...
        if (hiRes && number >= 0 && number < 32) {
          const lsbFirst = this.interface.getElementAttribute(path, `${this.kind}.lsbFirst`, false)
          if (lsbFirst) {
            Midi.send(this.portName, type, {channel: this.channel, controller:number + 32, value:(value >> 7) & 0x7F})
            Midi.send(this.portName, type, {channel: this.channel, controller:number, value:(value >> 0) & 0x7F})
          } else {
            Midi.send(this.portName, type, {channel: this.channel, controller:number, value:(value >> 7) & 0x7F})
            Midi.send(this.portName, type, {channel: this.channel, controller:number + 32, value:(value >> 0) & 0x7F})
          }
        } else {
          Midi.send(this.portName, type, {channel: this.channel, controller:number, value})
        }
      }
    }
